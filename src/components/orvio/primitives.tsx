...
export function Diamond({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="none" aria-hidden="true">
      <path d="M12 2L22 12L12 22L2 12L12 2Z" fill="url(#og-diamond)" />
      <path d="M12 2L22 12L12 22L2 12L12 2Z" stroke="rgba(255,255,255,0.2)" strokeWidth="0.5" />
      <defs>
...
export function SectionHeader({
  ...
  return (
    <div className={`max-w-3xl ${center ? "mx-auto text-center" : ""}`}>
      {eyebrow && (
        <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-indigo/30 bg-indigo/10 px-3 py-1 text-[11px] font-medium uppercase tracking-wider text-indigo">
          {eyebrow}
        </div>
      )}
      <h2 className="font-display text-3xl font-extrabold leading-[1.05] sm:text-5xl">{title}</h2>
      {subtitle && (
        <p className={`mt-4 text-base text-text-muted sm:text-lg ${center ? "mx-auto" : ""}`}>
          {subtitle}
        </p>
      )}
    </div>
