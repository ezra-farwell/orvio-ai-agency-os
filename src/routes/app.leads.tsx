...
function Leads() {
  ...
  return (
    <>
      <PageHeader title="Leads" sub={`${rows.length} of ${leads.length} matching · last 30 days`}
        actions={
          <>
            <select className="h-9 rounded-lg border border-border bg-background px-3 text-[13px]" value={client} onChange={e=>setClient(e.target.value)}>
              <option value="all">All clients</option>{clients.map(c => <option key={c.id}>{c.name}</option>)}
            </select>
